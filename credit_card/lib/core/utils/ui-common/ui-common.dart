
import 'dart:ui';

class CreditCardColor {
  static Color greySemiBold = Color(0xFF808080);
  static Color qreyBold = Color(0xFF484D50);
}

class CreditCardText {
  static String TEXT_CONTENT_SNACK_BAR_FAILURE = "Numero do cartão invalido";
  static String TEXT_ACTION_SNACK_BAR_FAILURE = "Limpar";
  static String TEXT_DEFAULT_NUMBER_CARD = "XXXX XXXX XXXX XXXX";
  static String TEXT_DEFAULT_CARD_HOLDER = "CARD HOLDER";
  static String TEXT_DATE_CARD = "MM/YY";
}